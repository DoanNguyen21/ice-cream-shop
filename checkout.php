<?php 
    ...
    if (isset($_POST['place_order'])) {
        ...
        $verify_cart = $conn->prepare("SELECT * FROM `tbl_cart`WHERE user_id =?");
        $verify_cart->execute([$user_id]);

        if (isset($_GET['get_id'])) {
            
            $get_product = $conn->prepare("SELECT * FROM `tbl_products` WHERE id=? LIMIT 1");
            ...
            if ($get_product->rowCount() > 0) {
                while($fetch_p = $get_product->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $insert_order = $conn->prepare("INSERT INTO `tbl_orders`(id, user_id, seller_id, name, 
         ...
         while($f_cart = $verify_cart->fetch(PDO::FETCH_ASSOC)){
            $s_products = $conn->prepare("SELECT * FROM `tbl_products` WHERE id =? LIMIT 1");
            ...
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
            $insert_order->execute([uniqid(), $user_id, $seller_id, $name, $number, $email, 
            $address, $address_type, $method, $f_cart['product_id'], $f_cart['price'], $f_cart['qty']]);
...
<head>
    ...
    <title>IceCream-GinD - checkout page</title>
                ...
                <div class="flex">
                    <div class="box">
                        ...
                        <div class="input-field">
                            ...
                            <input type="email" name="email" required maxlength="50" placeholder="Enter your number"
                                class="input">
                        </div>
                        <div class="input-field">
                    ...
                    <div class="box">
                        <div class="input-field">
                            ...
                            <input type="text" name="flat" required maxlength="50"
                                placeholder="e.g flat or bulding name" class="input">
                        </div>
                        <div class="input-field">
                        ...
                        <div class="input-field">
                            ...
                            <input type="number" name="pin" required maxlength="6" min="0" placeholder="e.g 110011 name"
                                class="input">
                        </div>
                    </div>
                </div>
                ...
                <div class="box-container">
                    <?php 
                       ...
                       if (isset($_GET['get_id'])) {
                        ...
                        $select_get = $conn->prepare("SELECT * FROM `tbl_products`WHERE id =?");
