<?php 
    ...
    if (isset($_POST['update_cart'])) {
        ...
        $update_qty = $conn->prepare("UPDATE `tbl_cart` SET qty =? WHERE id =?");
    ...
    if (isset($_POST['delete_item'])) {
        ...
        $verify_delete_item = $conn->prepare("SELECT * FROM `tbl_cart` WHERE id=?");
        $verify_delete_item->execute([$cart_id]);

        if ($verify_delete_item->rowCount() > 0 ) {

            $delete_cart_id = $conn->prepare("DELETE FROM `tbl_cart` WHERE id =?");
    ...
    if (isset($_POST['empty_cart'])) {
        ...
        $verify_empty_item = $conn->prepare("SELECT * FROM `tbl_cart` WHERE user_id=?");
...
<head>
    ...
    <title>IceCream-GinD - user cart page</title>
            ...
            <div class="button">
                <form action="" method="POST">
                    <button type="submit" name="empty_cart" class="btn"
                        onclick="return confirm('are you sure empty your cart')">empty cart</button>
                </form>
